...
function fRefreshRandomImage(){
    ...
    var checkUser;
    if (window.Ajax){
        checkUser = new Ajax.Request(sUrl,{method:'get',onSuccess:getRandomImage,onFailure:errAlert});
    }else{
        checkUser = $.ajax({url:sUrl,type:'GET',success:getRandomImage,error:errAlert});
    }
}

function getRandomImage(originalRequest){
	var sText = originalRequest.responseText||originalRequest;
	if(sText.indexOf('error')>-1){return;}
	else{
		var oImg = document.getElementById('randomNoImg');
		if ( !oImg ){
			...
			document.getElementById('checkcode').appendChild(oImg);
		}
		document.getElementById('randomNoImg').src = '/services/getimg?id=' + sText;
		document.getElementById('syscheckcode').value = sText;
	}
}
