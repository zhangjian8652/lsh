...
DateInput.prototype = {
		is_special_day : function (val) {
			for(var i in this.special_day){
		        if(this.special_day[i]==val){
		            return true;
		        }
		    }
		    return false;
  ...
  build: function() {
    ...
    this.dateSelector = this.rootLayers = $('<div id="date_selector" class="date_selector"></div>').append(nav, tableShell).appendTo(this.input);
  ...
  selectMonth: function(date) {
    ...
    if (!this.currentMonth || !(this.currentMonth.getFullYear() == newMonth.getFullYear() &&
      ...
      for (var i = 0; i <= numDays; i++) {
        ...
        var special_day = this.is_special_day(this.dateToString(currentDay)) ? ' special_day ' : '';
        if (this.isFirstDayOfWeek(currentDay)) dayCells += "<tr>";
        
        if (currentDay.getMonth() == date.getMonth()) {
        	if (Date.parse(currentDay) < now) {
        		dayCells += '<td class="selectable_day_before ' + special_day + ' " date="' + this.dateToString(currentDay) + '">' + currentDay.getDate() + '</td>';
        	} else {
        		dayCells += '<td class="selectable_day ' + special_day + ' " date="' + this.dateToString(currentDay) + '">' + currentDay.getDate() + '</td>';
        	}
        } else {
